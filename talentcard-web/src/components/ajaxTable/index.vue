  ...
  <div class="ajax-table-container">
    ...
    >
      <el-table-column
        ...
        :selectable="
          (row, index) =>
            tableData.selectable && tableData.selectable(row, index)
        "
      ...
      <template v-for="item in tableData.arr">
        ...
        >
          <template slot-scope="scope">
            <div
              v-if="item.type === 'img'"
              @click.stop="item.show && item.show(scope.row)"
            >
              <span class="view-img" @click="showImg(scope.row[item.prop])"
                >查看</span
              >
            </div>
            ...
            <div v-else-if="item.type === 'connect'">
               <span>{{scope.row[item.startProp] | slice}} - {{scope.row[item.endProp] | slice}}</span>
            </div>
            <div v-else-if="item.type === 'date'">
              <span v-if="scope.row[item.prop]">
                <span v-if="item.fmt === 0">{{
                  scope.row[item.prop] | (time - formater - year - month)
                }}</span>
                <span v-else>{{
                  scope.row[item.prop] | (time - formater - has - hour)
                }}</span>
              </span>
              <span v-else>-</span>
            </div>
            <div v-else-if="item.type === 'money'">
              {{ scope.row[item.prop] | (format - money) }}
            </div>
            <div
              ...
              :class="
                scope.row[item.prop] !== null
                  ? item.statusMap[Number(scope.row[item.prop])] &&
                    item.statusMap[Number(scope.row[item.prop])].class
                  : ''
              "
            ...
            <div v-else-if="item.type === 'switch'">
              <p v-if="item.show ? item.show(scope.row) : true">
                ...
                <el-switch
                  ...
                  :active-value="
                    item.hasOwnProperty('activeValue') ? item.activeValue : true
                  "
                  :inactive-value="
            ...
            <div v-else-if="item.type === 'handle'">
              <template v-for="(btn, index) in item.handles">
                ...
                >
                  <a
                    v-if="
                      isIE && /\.(png|jpg|PNG|JPG)$/.test(scope.row[btn.prop])
                    "
                ...
                >
                <!-- 按钮不显示时的默认值，有时候需要展示‘-’ -->
                <span v-else-if="btn.defaultValue" :key="index">{{
                  btn.defaultValue
                }}</span>
              </template>
            </div>
            <div v-else-if="item.type === 'card'">
              {{ scope.row[item.prop] | card }}
            </div>
            ...
            <div v-else-if="item.type === 'numbered'">
              ...
              {{
                ((listState &&
                  listState.pageSize * (listState.pageIndex - 1)) ||
                  0) +
                  scope.$index +
                  1
              }}
            ...
            <template v-else>{{
              (item.text && item.text(scope.row)) || textShow(scope.row, item)
            }}</template>
          </template>
        </el-table-column>
      </template>
...
* import AjaxTable from '@/components/ajaxTable/index' * @param {Array} list
表格数据 * @param {Boolean} loading loading状态 * @param {Object} tableData
表格配置信息 * @param id 表格id对应的字段 * @param arr 表格每一列的配置 * @param
{String} label 当前列的表头 * @param {String} prop 表格字段 * @param {String}
type 类型 （img（图片） || date（时间） || money（金额） || status（状态） ||
switch（开关） || handle（操作） || custom (自定义类型)） * @param {String}
width 宽度 * @param {Number} hide 隐藏当前列 * @param {Array} statusMap
状态对应的数组[{label: 'xxx'}] -- status对应数组的下标 -- type为status时必传 *
@param {String} switchText 开关提示文字 -- type为switch时必传 * @param
{Function} change 切换开关时的回调函数 -- type为switch时必传 * @param {Array}
handles 操作按钮的配置 -- type为handle时必传 * @param {Function} fn
点击按钮的回调函数 * @param {String} text 按钮文字 * @param {Function} show
按钮显示于隐藏的回调函数 * @param type 按钮的颜色（primary || danger || warning
|| info || success） * @param defaultValue: 文本默认值 */
<script>
