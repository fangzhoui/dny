  ...
  <div class="wrapper">
    <el-row class="subtitlerow">
      ...
      <el-button
        ...
        type="primary"
        class="addbtn"
        @click="gotoAdd('/Edit/quality')"
        v-if="times < 3&& tabs == 1"
      >新增职业资格信息</el-button>
    </el-row>
    <el-divider></el-divider>
    <el-row type="flex" justify="center" align="middle" v-if="qualityList.length == 0">
      <div class="noinfo">当前没有职业资格信息</div>
    </el-row>
    <template v-for="(item, index) in qualityList">
      <el-divider :key="index" content-position="center" v-if="index != 0">职业资格信息</el-divider>
      <el-row :key="index" class="formRow">
        <el-col :span="8">
          <span class="label">职业资格：</span>
          <span class="labelinfo">{{ item.category | qualityFilter }}</span>
        </el-col>
        <el-col :span="8">
          <span class="label">职业资格信息：</span>
        ...
        <el-col :span="8">
          <div class="btnGroup">
            <el-button
              type="primary"
              class="editbtn"
              @click="gotoEdit('/Edit/quality', index)"
              v-permission="'verifiedTalentUpdate'"
               v-if="tabs!=2"
            >编辑</el-button>
            <el-button
      ...
      <el-row :key="index" class="formRow">
        <span class="label">职业资格信息证明照片/扫描件：</span>
        <el-link type="primary" @click.native="showImg(item)">查看</el-link>
      </el-row>
    </template>
    <el-dialog title="职业资格对应政策权益" :visible.sync="dialogVisible" width="30%">
      <el-form ref="cardInfo" :model="qualityInfo" label-width="100px" v-loading="loading">
        <el-form-item label="职业资格：">{{ qualityInfo.quality | qualityFilter }}</el-form-item>
...
import { findPolicy, deleteQuality } from "../../../api/api";
...
export default {
  ...
  methods: {
    ...
    showBenefits(item) {
      ...
      this.qualityInfo.quality = item.category;
      this.getPolicy();
    },
    showImg(item) {
    ...
    async getPolicy() {
      try {
        this.loading = true;
        let para = {
          qualityList: [this.qualityInfo.quality],
        };
